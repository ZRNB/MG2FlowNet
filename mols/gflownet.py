...
max_deepth = 10 # 单次模拟的最大深度
epsilon = 0.2 # 表示MCTS算法的占比
...
class TreeNode:
    ...
    def __init__(self, mol, is_terminal):
        ...
        self.is_terminal = is_terminal #每次增加节点的时候记得更新
    ...
    def get_value(self, c_puct, a): # 此处的a是没有分到各个反应位点的a
...
class Dataset:
    ...
    # SIMULATION
        ...
        if node.mol.numblocks >= self.min_blocks and selected_id == 0:
            ...
            # print('扩展选中的action是:',*action,' ','扩展选中的m是:',m_copy.smiles)
    ...
    def get_move(self, n_playout, path, m, current_step):
        ...
        q_min = q_values.min()
        q_max = q_values.max()
        normalized_q = (q_values - q_min) / (q_max - q_min + 1e-8)  # 加小量防止除以0
    ...
    def _get_sample_model(self, current_step):
        ...
        for t in range(max_blocks):
            ...
            if t >= self.min_blocks and action == 0:
                ...
                m_old = m
                # m = self.mdp.add_block_to(m, *action) 
                m = state_node_map[m.smiles]._children[action_id].mol # 采用MCTS树上的节点进行动作的执行
                path.append(state_node_map[m.smiles])
                if len(m.blocks) and not len(m.stems) or t == max_blocks - 1:
                    # can't add anything more to this mol so let's make it
                    # terminal. Note that this node's parent isn't just m,
...
def log_resources(step: int):
    ...
    mem_gb = process.memory_info().rss / (1024**3)   # 进程独占内存
...
def main(args):
    print("n_playout = 5,initial_epsilon=0,final_epsilon=0.1,step=1000,get_value=q+u**5,get_move通过q**5来选择")
    # print("n_playout = 1,epsilon=0.1,get_value=q+u**5,get_move通过q**5来选择")
    pid = os.getpid()
