...
c_puct = 1 # 探索率
n_playout = 1 # MCTS迭代轮数
max_deepth = 20 # 单次模拟的最大深度
epsilon = 0.2
# initial_epsilon = 0 # 表示MCTS算法权重的占比
# final_epsilon = 0.2
...
class TreeNode:
    """MCTS树节点"""
    # is_terminal_node只是用来表示是否是由于终止动作导致的终止节点，并不表示节点是否是达到边缘状态
    def __init__(self, state_tuple, is_terminal_node):
        self._parent = []  # {parent_node: prior_p}：prior_p是对应父节点到本节点的概率，即本节点在这个父节点上的先验概率
        self._children = {}  # {action: child_node}
        self._n_visits = 0
        self._Q = 0.25625  # 初始化奖励值为0.1
        self.state_tuple = state_tuple  # 保存状态元组
        self.is_terminal_node = is_terminal_node  # 是否是终止节点
    ...
    def select(self, env, c_puct, agent):
        if self.is_terminal(env) or self.is_terminal_node:# 选择到了终止节点
            return self, -2
        ...
        exp_values = np.exp(puct_values - np.max(puct_values))  # 数值稳定性处理
        probs = exp_values / np.sum(exp_values)
        # 根据概率分布随机选择一个动作
        # print('当前选择的节点下的孩子节点的概率分布是：',probs, end=' ')
        selected_idx = np.random.choice(len(actions), p=probs)
        return self._children[actions[selected_idx]], actions[selected_idx]

    # 当前状态执行a动作的价值，a =0,1,2    
    def get_value(self, c_puct, a, agent):
        # 原版：使用GFN的pf# PUCT(s,a) = Q(s,a) + c_puct * P(s,a) * sqrt(sum(N(s,b)) / (1 + N(s,a)))
        # probs = torch.softmax(agent.model(tf(agent.envs[0].obs(self.state_tuple))), dim=-1)
        # action_probs = probs.tolist()
        # p = action_probs[a]
        # 新版：使用平均概率
        num_actions = len(self._children)  # 可用动作数量
        p = 1.0 / num_actions             # 均匀分布概率
...
class FlowNetAgent:
    ...
    def playout(self, env, func, root_node, path):
        ...
        #     print("没有在树上找到对应的节点")
        # else:
        #     print("本次要进行迭代的节点是：",state_tuple)
        # path_str = ''
        # for node in path:
        #     path_str += str(node.state_tuple)
        # print('选择阶段前的path_str is :',path_str)
        env_copy = copy.deepcopy(env)
        count = 0
        while True:
            best_node, best_action = node.select(env_copy, c_puct, self)
            if best_action == -2: # 选择到了终止节点
                reward = env_copy.s2r(np.array(node.state_tuple))
                # path.append(best_node)
                # print('选择到了终止节点:',best_node.state_tuple,'分数是:',reward)
                best_node.update_recursive(reward, path)
                for _ in range(count):
                    path.pop()
                return 
                
            if best_action == -1: # 选择到了未完全扩展的节点
                # path.append(best_node)
                # print('选择到了未扩展完全的节点:',best_node.state_tuple)
                break
            # 选择到了扩展完全的节点，所以继续选择
            path.append(best_node)
            count += 1
        ...
        if unexplored:
            for action, prob in unexplored:
                ...
                else:  # 终止动作，得到的节点的状态肯定跟自己是一样的，所以创建节点，直接加上去就行
            ...
            # print("模拟阶段前的path:")
            # for node in path: 
            #     print(node.state_tuple,end='')
            # print()
            for _ in range(count + 1):
                if path:
                    path.pop()
            # print("模拟阶段后的path:",end=' ')
            # for node in path:
            #     print(node.state_tuple,end='')
            # input()
    ...
    def evaluate_rollout(self, env):
        ...
        for i in range(max_deepth):  # 最大深度
            s = env.obs(state)
            with torch.no_grad():
    ...
    def get_move(self, env, root_node, path, current_step):#
        ...
        with torch.no_grad():
            ...
            pf = torch.softmax(logits, dim=-1).cpu().numpy()  # 转换为概率分布
    ...
    def sample_many(self, mbsize, all_visited, root_node, current_step):
        # print('这轮采样的根节点是:',root_node.state_tuple)
        batch = []
        ...
        while not all(done):
            ...
            for i, e in enumerate(active_envs):
                ...
                # path_str = ''
                # for node in paths[active_indices[i]]:
                #     path_str += str(node.state_tuple)
                # print('第',i,'个环境动完后path is:',path_str)
            step = [i.step(a) for i,a in zip([e for d, e in zip(done, self.envs) if not d], acts)]
            ## 调试
            # for i, (sp, r, d, a) in enumerate(step):
            #     print('第',i,'个环境执行完get_move的动作后得到的状态是:', a)
        ...
        # if all_visited:
        #     print('这轮采样后所有访问过的状态是:',all_visited)
        return batch
...
def main(args):
    ...
    for i in tqdm(range(args.n_train_steps+1), disable=not args.progress):
        ...
        if not i % 100:
            ...
            #     for j in range(32):
            #         for k in range(32):
            #             score += str(round(state_node_map[tuple(np.array([i,j,k]))]._Q, 3) if tuple(np.array([i,j,k])) in state_node_map else -1)
            #             score += ' '
            #     print(score)
            # print()
            if args.progress:
